...
void __w3cexcept(const char *szaddress, long nport, W3Client::w3t t, const char *szuri, const char *szmsg){
  ...
  ::LocalFree(lpMsgBuffer);
        ...
        void W3Client::InitializePostArguments(){
          ...
          contents_ = "Content-Type: application/x-www-form-urlencoded\r\n";
          accepts_ = "Accept: */*;";
          return;
        }
        ...
        unsigned long W3Client::GetPostData(unsigned  char * &buf, unsigned long len){
          ...
          buf = (unsigned  char *)postBuf_;
          return postBufLen_;
        }

        bool W3Client::RequestPost(const char *szuri, const char *szref /*=NULL*/){

          static LPCTSTR szAcceptType=accepts_;
          static LPCTSTR szContentType=contents_;
          ...
          // REPLACE HEADER
          if(!::HttpAddRequestHeaders( _hRequest, accepts_, -1L, HTTP_ADDREQ_FLAG_REPLACE))
            throw "additional header failed...";

          if(!::HttpAddRequestHeaders( _hRequest, contents_, -1L, HTTP_ADDREQ_FLAG_ADD))
            throw "additional header failed...";

          if(!::HttpAddRequestHeaders( _hRequest, actionHeader_.c_str(), -1L, HTTP_ADDREQ_FLAG_ADD))
            throw "additional header failed...";

                 std::string cl="Content-Length: ";
                 char tmp[10];
                 sprintf(tmp,"%d",postBufLen_);
                 cl+=tmp;
                 cl+="\r\n";
          if(!::HttpAddRequestHeaders( _hRequest, cl.c_str(), -1L, HTTP_ADDREQ_FLAG_ADD))
            throw "additional header failed...";

        //       std::string headers = contents_ ;
        //       headers+=actionHeader_ + cl;
          ...
          if(!::HttpSendRequest( _hRequest,     // handle by returned HttpOpenRequest
                                 NULL, // additional HTTP header
                                  NULL, // additional HTTP header length
...
bool
W3Client::GetFile(const char *szuri, const char *szfile, bool ascii /*=false*/){
...
unsigned long
W3Client::GetFile(const char *szuri, unsigned char *buf, unsigned long len, bool ascii /*=false*/){
...
bool
 W3Client::PutFile(const char *szuri, const char *szfile, bool ascii /*=false*/){
...
unsigned long
 W3Client::PutFile(const char *szuri, unsigned char *buf, unsigned long len, bool ascii){
